...
class LineFinder {
  ...
  public:
	  ...
	  // Draw lines filtering horizontal lines on an image, filtering range determined by Btheta (rads)
	  void drawFilterHLines(cv::Mat &image, double Btheta, cv::Scalar color=cv::Scalar(255)) {
		
		  // Draw the lines
		  std::vector<cv::Vec4i>::const_iterator it2= lines.begin();
	
		  while (it2!=lines.end()) {
		      double Ymag = abs(((*it2)[3]+shiftV) - ((*it2)[1]+shiftV));
			  double Xmag = abs(((*it2)[2]+shiftV) - ((*it2)[0]+shiftV));
			  
			  if ( atan2(Ymag,Xmag) > Btheta){
				  cv::Point pt1((*it2)[0]+shiftH,(*it2)[1]+shiftV);        
				  cv::Point pt2((*it2)[2]+shiftH,(*it2)[3]+shiftV);
				  cv::line( image, pt1, pt2, color, 6 );
			  }			  
